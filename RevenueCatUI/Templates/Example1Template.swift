...
struct Example1Template: TemplateViewType {
    ...
    var body: some View {
        ...
        case let .success(data):
            Example1TemplateContent(data: data, introEligibility: self.introEligibility)
                .task(id: EligibilitySource(package: self.package, checker: self.introEligibilityChecker)) {
    ...
    // Equality of this `struct` allows `.task` to determine whether it needs to re-run the task
    private struct EligibilitySource: Equatable {
        let package: Package?
        let checker: TrialOrIntroEligibilityChecker

        static func == (lhs: EligibilitySource, rhs: EligibilitySource) -> Bool {
            return lhs.package == rhs.package && lhs.checker === rhs.checker
        }
    }
